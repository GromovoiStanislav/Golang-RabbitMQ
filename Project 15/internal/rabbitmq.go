...
func ConnectRabbitMQ(url string) (*amqp.Connection, error) {
	//conn, err := amqp.Dial(fmt.Sprintf("amqps://%s:%s@%s/%s", username, password, host, vhost))
	conn, err := amqp.Dial(fmt.Sprintf(url))
	if err != nil {
		return nil, err
	}
	return conn, nil
...
// CreateExchange will create a new exchange
func (rc RabbitClient) CreateExchange(exchangeName, exchangeType string, durable, autodelete bool) error {
    err := rc.ch.ExchangeDeclare(exchangeName, exchangeType, durable, autodelete, false, false, nil)
    return err
}
...
func (rc RabbitClient) Send(ctx context.Context, exchange, routingKey string, options amqp.Publishing) error {
	...
	log.Println("confirmation - ",confirmation.Wait())
	return nil
}
